...
// custom comparator for greedyMethod2
bool cmpForGreedy1(pair<int,int>p,pair<int,int>q) {
    // if weight of two items are equal then we should choose the greater
    // profit so that we can maximize profit.
    // otherwise we will always choose the items with the lower weight
...
// way 2 : items having minimum weight will be taken first
void greedyMethod2(vi profit,vi weight,int knapsackSize) {
    ...
    while(knapsackSize > 0) {
        ...
        if(currentItem.second <= knapsackSize) {
            ...
            double p = knapsackSize*((double)currentItem.first/currentItem.second);
            maxProfit += ceil(p);
            ans.push_back(make_pair(ceil(p),knapsackSize));
            knapsackSize = 0;
        }
        itemIndex++;
    }
...
int main() {
    ...
    greedyMethod2(profit,weight,knapsackSize);
